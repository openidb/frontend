...
# Upload files
scp -P $POD_PORT book-viewer/training/data/combined_training.jsonl root@$POD_IP:/workspace/training/
scp -P $POD_PORT book-viewer/training/data/gold_standard_evaluation.jsonl root@$POD_IP:/workspace/training/
scp -P $POD_PORT book-viewer/training/scripts/train_bge_m3.py root@$POD_IP:/workspace/training/
```
...
```bash
cd book-viewer
bun run scripts/generate-embeddings.ts --model=bge-m3 --collection=quran
bun run scripts/generate-embeddings.ts --model=bge-m3 --collection=hadith
```
