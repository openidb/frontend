...
let deleteExpiredStmt: Database.Statement | null = null;
...
function initDb(): boolean {
  ...
  try {
    ...
    deleteExpiredStmt = db.prepare("DELETE FROM embeddings WHERE created_at < ?");
...
export function cleanupExpiredEntries(): number {
  if (!initDb() || !deleteExpiredStmt) return 0;
  ...
  try {
    ...
    console.error("[PersistentEmbeddingCache] Error cleaning up expired entries:", err);
    return 0;
  }
}
 ...
 */
export function getPersistentCacheStats(): { count: number; sizeBytes: number } | null {
  ...
  try {
    ...
    console.error("[PersistentEmbeddingCache] Error getting stats:", err);
    return null;
  }
}
