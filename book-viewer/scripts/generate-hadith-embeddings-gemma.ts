 ...
 *
 * Usage:
 *   bun run scripts/generate-hadith-embeddings-gemma.ts [options]
 *
 * Options:
 ...
 *   --collections=a,b    Only process specific collections (comma-separated slugs)
 *
 * Examples:
 *   bun run scripts/generate-hadith-embeddings-gemma.ts --collections=hisn --force
 *   bun run scripts/generate-hadith-embeddings-gemma.ts --collections=bukhari,muslim
 */
...
// Parse --collections filter (comma-separated list of collection slugs)
const collectionsArg = process.argv.find((arg) => arg.startsWith("--collections="));
const collectionsFilter: string[] | null = collectionsArg
  ? collectionsArg.split("=")[1].split(",").map((s) => s.trim())
  : null;

type HadithWithRelations = {
  id: number;
  hadithNumber: string;
  textArabic: string;
  textPlain: string;
 ...
 */
function getTextForEmbedding(hadith: HadithWithRelations): string {
  const slug = hadith.book.collection.slug;

  // Hisn al-Muslim: combine chapter title + text for better semantic search
  if (slug === "hisn" && hadith.chapterArabic) {
    return normalizeArabicText(`${hadith.chapterArabic}، ${hadith.textPlain}`);
  }

  return normalizeArabicText(hadith.textPlain);
}
...
async function main() {
  ...
  if (collectionsFilter) {
    console.log(`Filter: ${collectionsFilter.join(", ")}`);
  }
  console.log();
  ...
  const whereClause = collectionsFilter
    ? { book: { collection: { slug: { in: collectionsFilter } } } }
    : {};
